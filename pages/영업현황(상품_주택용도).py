...
PRODUCT_GROUP_ORDER = ["공동주택", "단독주택", "일반용", "업무용", "산업용", "열병합용"]
...
def split_home_group(group_series: pd.Series, usage_series: pd.Series = None) -> pd.Series:
    """'가정용'으로 매핑된 항목을 용도 기준으로 공동주택/단독주택으로 재분류.
    용도 == '공동주택' -> 공동주택, 그 외(또는 용도컬럼 없음) -> 단독주택"""
    group_series = group_series.copy()
    mask_home = group_series == "가정용"
    if not mask_home.any():
        return group_series
    if usage_series is not None:
        usage_clean = usage_series.map(remove_paren_codes)
        is_apt = usage_clean == "공동주택"
    else:
        is_apt = pd.Series(False, index=group_series.index)
    group_series = group_series.mask(mask_home & is_apt, "공동주택")
    group_series = group_series.mask(mask_home & ~is_apt, "단독주택")
    return group_series
...
def get_agg_by_product(df: pd.DataFrame, mode="net", product_col="상품명") -> pd.Series:
    ...
    # 가정용 -> 공동주택/단독주택 분리 (용도 컬럼 기준)
    usage_series = df['용도'] if '용도' in df.columns else None
    df[product_col] = split_home_group(df[product_col], usage_series)
...
def build_use_change_product_pivot(snap_date: date):
    ...
    if in_col:
        ...
        usage_in = df_in['변경용도'] if '변경용도' in df_in.columns else None
        df_in[in_col] = split_home_group(df_in[in_col], usage_in)
    ...
    if out_col:
        ...
        usage_out = df_out['이전용도'] if '이전용도' in df_out.columns else None
        df_out[out_col] = split_home_group(df_out[out_col], usage_out)
...
# ───────────────────────────────
import os
EXPLANATION_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "작성방법", "영업현황(상품_주택용도)_작성방법.md")
